...
import { getTrainingSession, updateTrainingSession } from "@/services/api/training-sessions";
...
import type { UpdateTrainingSessionData } from "@/services/api/training-sessions";
...
export default function EditTrainingSessionPage() {
    ...
    const { data: session, isLoading } = useEntity<TrainingSession>({
    ...
    const {
        ...
        isSubmitting,
        error,
    } = useEntityForm<TrainingSession, UpdateTrainingSessionData, UpdateTrainingSessionData>({
        initialData: {
            title: "",
            start_date: "",
            end_date: "",
        ...
        updateFunction: async (entityId, data) => {
            const res = await updateTrainingSession(entityId, {
        ...
        onSuccess: () => router.push(`/dashboard/training-sessions/${id}`),
        validate: (data) => {
    ...
    useEffect(() => {
        if (session) {
            setFormData({
                ...
                department_id: session.department_id ?? undefined,
    ...
    return (
        <ProtectedPage permission={PERMISSIONS.TRAINING_SESSIONS.EDIT}>
            <div className="max-w-2xl mx-auto space-y-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">Edit training session</h1>
                ...
                <FormContainer onSubmit={handleSubmit} error={error}>
                    ...
                    <FormField
                        ...
                        rows={4}
                    />
                    <ActionButtons submitLabel="Update session" isSubmitting={isSubmitting} />
                </FormContainer>
            </div>
        </ProtectedPage>
    );
